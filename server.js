...
const upload = multer({
  ...
  limits: { fileSize: 10 * 1024 * 1024 } // ⚠️ Vercel safe limit ~10MB
});
...
app.post("/upload/v1", upload.single("file"), async (req, res) => {
  try {
    ...
    // simpan buffer di memory (⚠️ sementara)
    fileMap[fakeName] = {
      buffer,
      createdAt: Date.now()
    };
    ...
    res.json({
      success: true,
      url: `/upload/v1/${fakeName}/${token}?preview=true`
    });
...
app.post("/api/upload.php", upload.single("file"), async (req, res) => {
  try {
    ...
    uguuMap[fakeName] = {
      url: uguuRes.data.files[0].url
    };

    res.json({
      success: true,
      url: `/api/upload/${fakeName}/${token}?preview=true`
    });
...
// MEMORY FILE SERVE
// =======================
app.get("/upload/v1/:file/:token", (req, res) => {
  const data = fileMap[req.params.file];
  if (!data) return res.status(404).send("Not found");
